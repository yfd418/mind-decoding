...
import { X, Mail, Lock, TreeDeciduous, Check, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose, initialMode = 'login', isDarkMode }: ExtendedAuthModalProps) {
  ...
  const [selectedForestName, setSelectedForestName] = useState('');
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const { login, registerWithForestName, resendConfirmation } = useAuth();

  useEffect(() => {
    if (isOpen) {
      setMode(initialMode);
      if (initialMode === 'register') {
        setSelectedForestName(generateForestName());
      }
    }
  }, [isOpen, initialMode]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        const result = await registerWithForestName(email, password, selectedForestName, agreedTerms);
        if (result.success) {
          if (result.needsConfirmation) {
            setSuccess('注册成功！请查收邮件并点击验证链接完成注册。');
          } else {
            onClose();
            resetForm();
          }
    ...
    <div className="fixed inset-0 z-[100] flex items-start sm:items-center justify-center p-4 overflow-y-auto">
      ...
      >
        ...
        <div className="p-6">
          ...
          {mode === 'register' && selectedForestName && (
            <div className={`mb-4 p-3 rounded-xl text-center ${
              ...
              <button
                type="button"
                onClick={() => setSelectedForestName(generateForestName())}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {success && (
              <div className={`p-2.5 rounded-lg text-sm ${
                ...
                {(success.includes('验证') || success.includes('邮件')) && (
                  <button
                    type="button"
                    onClick={async () => {
                    ...
                    className={`mt-2 text-xs underline ${
                      isDarkMode ? 'hover:text-green-200' : 'hover:text-green-700'
                    }`}
                  >
                    没收到？点击重新发送验证邮件
                  </button>
